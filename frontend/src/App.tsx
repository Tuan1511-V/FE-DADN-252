            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs font-medium text-zinc-500 bg-zinc-800/50 px-3 py-1.5 rounded-full border border-zinc-800">
                  {overview.summary.sensorsOnline} Online | {overview.summary.sensorsIssue} Issue
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {overview.sensors.map((device) => (
                  <React.Fragment key={device.device_code}>
          ...
          {activeTab === 'control' && (
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs font-medium text-zinc-500 bg-zinc-800/50 px-3 py-1.5 rounded-full border border-zinc-800">
                  {overview.summary.actuatorsOnline} Online | {overview.summary.actuatorsIssue} Issue
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {overview.actuators.map((device) => (
                  <React.Fragment key={device.device_code}>
                    <DeviceRow
                      device={device}
                      busy={busyDeviceCode === device.device_code}
                      onToggle={(nextValue) => void handleToggleDevice(device, nextValue)}
                    />
                  </React.Fragment>
                ))}
              </div>
